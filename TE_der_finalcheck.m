...
% f = besselj(1,kn*sqrt((x-a)^2 + (y-b)^2  ));
% pretty((diff(f,y)))
gst = 1i/4*besselj(0,k0*n*sqrt((x-a)^2 + (y-b)^2  ));
...
disp ('~~~~~~~~~~~~Start dx~~~~~~~~~~~~~~');
resx = diff(g_rot,x);
compx = -1i/4*(k0*n*(x-a)/(sqrt((x-a)^2 + (y-b)^2  ))*besselj(1,k0*n*sqrt((x-a)^2 + (y-b)^2))+1i*k0*OMEGAc*b*besselj(0,k0*n*sqrt((x-a)^2 + (y-b)^2  )))*rot;
disp ('dx - My result');
pretty(compx)
...
disp ('~~~~~~~~~~~~Start dy~~~~~~~~~~~~~~');
resy = diff(g_rot,y);
compy = -1i/4*(k0*n*(y-b)/(sqrt((x-a)^2 + (y-b)^2  ))*besselj(1,k0*n*sqrt((x-a)^2 + (y-b)^2  ))-1i*k0*OMEGAc*a*besselj(0,k0*n*sqrt((x-a)^2 + (y-b)^2  )))*rot;
% disp ('dy - My result');
% pretty(compy)
...
disp ('~~~~~~~~~~~~Start da~~~~~~~~~~~~~~');
resa = diff(g_rot,a);
compa = -1i/4*(k0*n*(a-x)/(sqrt((x-a)^2 + (y-b)^2  ))*besselj(1,k0*n*sqrt((x-a)^2 + (y-b)^2  ))-1i*k0*OMEGAc*y*besselj(0,k0*n*sqrt((x-a)^2 + (y-b)^2  )))*rot;
% disp ('dy - My result');
% pretty(compa)
% disp ('dy - MATLAB result');
% pretty(resa)
...
disp ('~~~~~~~~~~~~Start db~~~~~~~~~~~~~~');
resb = diff(g_rot,b);
compb = -1i/4*(k0*n*(b-y)/(sqrt((x-a)^2 + (y-b)^2  ))*besselj(1,k0*n*sqrt((x-a)^2 + (y-b)^2  ))+1i*k0*OMEGAc*x*besselj(0,k0*n*sqrt((x-a)^2 + (y-b)^2  )))*rot;
% disp ('dy - My result');
% pretty(compb)
% disp ('dy - MATLAB result');
% pretty(resb)
...
pretty(simplify((my_yy_1+my_yy_2)-simplify(-diff(diff(g_rot,b),y))))
disp ('~~~~~~~~~~~~dxy^~~~~~~~~~~~~~~');
my_xb_1 = -1i/4*k0*n*besselj(1,k0*n*rr)*(2*(b-y)*(a-x)/(rr^3)-(b-y)/rr*(1i*k0*OMEGAc*b)+1i*k0*OMEGAc*(a-x)/rr*x)*rot;
my_xb_2 = 1i/4*besselj(0,k0*n*rr)*((k0*n)^2*(b-y)*(a-x)/(rr^2)+(1i*k0*OMEGAc)^2*x*b-1i*k0*OMEGAc)*rot;

pretty((simplify((my_xb_1+my_xb_2)-simplify(diff(diff(g_rot,b),x)))))
disp ('~~~~~~~~~~~~dyx^~~~~~~~~~~~~~~');
my_ya_1 = -1i/4*k0*n*besselj(1,k0*n*rr)*(2*(b-y)*(a-x)/(rr^3)+(a-x)/rr*(1i*k0*OMEGAc*a)-1i*k0*OMEGAc*(b-y)*y/rr)*rot;
my_ya_2 = 1i/4*besselj(0,k0*n*rr)*((k0*n)^2*(b-y)*(a-x)/(rr^2)+(1i*k0*OMEGAc)^2*y*a+1i*k0*OMEGAc)*rot;
